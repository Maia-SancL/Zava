...
include 'c:/xampp/htdocs/Zava/public/componentes/php/header.php';
...
?>
<div class="layout">
    <?php include 'c:/xampp/htdocs/Zava/public/componentes/php/menuLateral.php';?>
    <main>
        <link rel="stylesheet" href="/Zava/cliente/css/productos.css">
         <?php include 'c:/xampp/htdocs/Zava/public/componentes/php/navegador.php';?>
         <section class="section-filtros">
            <form method="get" id="filtros-restaurantes" action="">
                ...
                <div class="btn-oferta">
                    <input type="checkbox" id="btn-switch" class="btn-switch" name="descuento" <?php echo $descuento==true ? "checked" : '';?>/>
    ...
    if ($result = $conexion->query($query)) {
        ...
        while ($fila = $result->fetch_assoc()) {
            $ruta_imagen= '/Zava/comercio/uploads/' . $fila['imagen'];
            ?>
                                <article data-href="/Zava/cliente/php/mostrarProducto.php?id_producto=<?php echo $fila['id_producto']; ?>" class="producto">
                    <div class="cont-img">
                        <img src="<?php echo $ruta_imagen;?>" alt="<?php echo $fila['nombre'];?>">
                        <div class="cont-oferta-favorito"> 
                            <?php if($fila['descuento'] == 1 && !empty($fila['porcentaje_descuento'])){?>                       
                            <div class="cont-oferta">
                                <p>%<?php echo $fila['porcentaje_descuento'];?> off</p>
                            </div>
                            <?php }; ?>
                    ...
                    <div class="informacion-producto"> 
                        <span class="tipo-producto"><?php echo $fila['categoria'];?></span>
                        <span class="titulo-producto"><?php echo $fila['nombre'];?></span>
                        <span class="peso-producto"><?php echo $fila['peso'];?></span>
                        <div class="cont-precio-btn">
                            <?php 
                            $precio_final = $fila['precio'];
                            $precio_original = null;

                            if($fila['descuento'] == 1 && !empty($fila['porcentaje_descuento']) && $fila['porcentaje_descuento'] > 0){
                                $precio_original = $fila['precio'];
                                $descuento_valor = $precio_original * ($fila['porcentaje_descuento'] / 100);
                                $precio_final = $precio_original - $descuento_valor;
                            ?>
                                <span class="precio-oferta">$<?php echo number_format($precio_original, 2, ',', '.')?></span>
                            <?php } ?>
                            
                            <span class="precio">$<?php echo number_format($precio_final, 2, ',', '.')?></span>
...
<?php 
include 'c:/xampp/htdocs/Zava/public/componentes/php/footer.php';
?>
<script src="/Zava/cliente/js/productos.js"></script>

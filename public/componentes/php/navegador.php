...
if (isset($_SESSION) && isset($_SESSION['tipo_usuario']) && isset($_SESSION['id'])):
    if ($_SESSION['tipo_usuario'] === 'Usuario'):
        ...
        $query = "SELECT nombre, nickname, imagen FROM usuarios WHERE id_usuario = $id_usuario";
        ...
        ?>
        <link rel="stylesheet" href="/Zava/public/css/navegador.css">
        <!-- Menú para clientes -->
        <nav class="nav">
            <div class="barra-buscar" id="contenedor-busqueda-nav" style="position: relative;">
                ...
                <input type="text" id="busqueda-nav" placeholder="Buscar" required>
            </div>
            <div class="cont-btns">
                <ul class="nav-lista-btns">
                    ...
                    ?>
                    <li class="btns cart-container" onclick="location.href='<?= BASE_URL ?>carrito'">
                        <?php if ($total_productos > 0): ?>
                            <span class="cart-count" id="cart-count"><?= $total_productos ?></span>
                        <?php endif; ?>
                        ...
                        <ul class="user-dropdown-content">
                            <li>
                                <div class="user-info">
                                    <div class="cont-user-img">
                                        <img src="/Zava/public/img/perfiles/<?= $foto ?>" alt="Foto de perfil" class="user-avatar">
                                    </div>
                            ...
                            <li>
                                <div class="btn-dropdown perfil">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" onclick="bandejaUsuario(event)"><circle cx="12" cy="6" r="4" fill="currentColor " class="nav-icono"/><path class="nav-icono"fill="currentColor" d="M20 17.5c0 2.485 0 4.5-8 4.5s-8-2.015-8-4.5S7.582 13 12 13s8 2.015 8 4.5"/></svg>
                                    <a onclick="location.href='<?= BASE_URL ?>perfil'" class="profile-btn">Ir a mi perfil</a>
                                </div>
                            </li>
                            <li>
                ...
                <div class="cont-btn-agregar-receta">
                    <button class="btn-agregar-receta" onclick="location.href='<?= BASE_URL ?>crear-receta'">
    ...
    <?php elseif ($_SESSION['tipo_usuario'] === 'Comercio'):
        ...
        $query = "SELECT nombre, imagen FROM usuarios WHERE id_usuario = $id_comercio";
        $resultado = mysqli_query($conexion, $query);
        if ($resultado && mysqli_num_rows($resultado) > 0) {
            $comercio = mysqli_fetch_assoc($resultado);
            $nombre = htmlspecialchars($comercio['nombre']);
            $foto = $comercio['imagen'] ? htmlspecialchars($comercio['imagen']) : 'perfil.png';
            $rutaImg = BASE_URL . "public/img/perfiles/" . $foto;
        } else {
            $nombre = 'Comercio';
            $rutaImg = BASE_URL . 'public/img/perfiles/perfil.png';
        }
    ?>
        <!-- Menú para comercios -->
        <link rel="stylesheet" href="/Zava/public/css/navegador.css">
        <nav class="nav">
            <div class="cont-btns">
                <ul class="nav-lista-btns">
                    <li class="btns" onclick="location.href='<?= BASE_URL ?>comercio/perfil'">Panel de Comercio</li>
                    <li class="btns user-dropdown">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" onclick="bandejaUsuario(event)"><circle cx="12" cy="6" r="4" fill="currentColor" class="nav-icono"/><path class="nav-icono" fill="currentColor" d="M20 17.5c0 2.485 0 4.5-8 4.5s-8-2.015-8-4.5S7.582 13 12 13s8 2.015 8 4.5"/></svg>
                        <ul class="user-dropdown-content">
                            <li>
                                <div class="user-info">
                                    ...
                                    <a onclick="location.href='<?= BASE_URL ?>comercio/perfil'" class="profile-btn">Ir a mi perfil</a>
                                </div>
                            </li>
                            <li>
                                <div class="btn-dropdown logout">
                ...
                <div class="cont-btn-agregar-receta">
                    <button class="btn-agregar-receta" onclick="location.href='<?= BASE_URL ?>comercio/agregar-producto'">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path class="btn-agregar-icono"fill="currentColor" d="M11 13H6q-.425 0-.712-.288T5 12t.288-.712T6 11h5V6q0-.425.288-.712T12 5t.713.288T13 6v5h5q.425 0 .713.288T19 12t-.288.713T18 13h-5v5q0 .425-.288.713T12 19t-.712-.288T11 18z"/></svg>
                        Crear Producto
                    </button>
                </div>
        ...
        <nav class="nav">
            <div class="cont-btns">
                <ul class="nav-lista-btns">
                    ...
                    <li class="btns"><a href="<?= BASE_URL ?>administrador/panel">Panel</a></li>
                    <li class="btns"><a href="<?= BASE_URL ?>administrador/panel/usuarios">Usuarios</a></li>
                    <li class="btns"><a href="#">Reportes</a></li>
                    <li class="btns"><a href="#">Configuración</a></li>
                    <li class="btns">
    ...
    <nav class="nav">
        ...
        <div class="cont-btns">
            <ul class="nav-lista-btns">
                <li class="btns">
                    <button class="iniciar-sesion" onclick="location.href='<?= BASE_URL ?>login'">
                ...
                <li class="btns">
                    <button class="registrarse" onclick="location.href='<?= BASE_URL ?>registro'">

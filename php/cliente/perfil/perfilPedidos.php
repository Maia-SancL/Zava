...
<link rel="stylesheet" href="/Zava/css/perfilPedidos.css">
    <main>
        ...
        <div class="cont-pedidos">
            <h3>Mis Pedidos</h3>
            <?php
            $query_pedidos = "SELECT id_pedido, fecha_pedido, total, estado FROM pedidos WHERE id_usuario = $id_usuario ORDER BY fecha_pedido DESC";
            ...
            if (mysqli_num_rows($resultado_pedidos) > 0) {
                ...
                while ($pedido = mysqli_fetch_assoc($resultado_pedidos)) {
                    ...
                    echo '        <p><strong>Pedido #:</strong> ' . htmlspecialchars($pedido['id_pedido']) . '</p>';

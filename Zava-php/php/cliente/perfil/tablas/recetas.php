...
<section class="section-tabla-productos">
    ...
    <?php while ($receta = mysqli_fetch_assoc($result)): ?>
        <article class="producto" style="cursor:pointer;" onclick="if(event.target.tagName !== 'BUTTON' && event.target.tagName !== 'FORM' && event.target.tagName !== 'A'){ window.location.href='/Zava-php/php/cliente/mostrarReceta.php?id=<?= $receta['id_receta'] ?>'; }">
            <div class="informacion-principal">
                <div class="informacion">
                    ...
                    <div class="btns">
                        <a href="/Zava-php/php/cliente/perfil/modificarReceta.php?id=<?= $receta['id_receta'] ?>" class="btn-modificar" onclick="event.stopPropagation();">✏️ Modificar</a>
